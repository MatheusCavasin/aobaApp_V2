...
class PedidosRepository {
    ...
    func confirmarEntrega(pedidoId: Int, completion: @escaping (Any?, Error?) -> Void) {
        let url = Singleton.shared.apiEndPoint + "/api/v1/pedido/\(pedidoId)/entregar"
        
        ApiResource.request(method: "PUT", url: url, params: nil, body: nil, withAuth: true) { (result, err) in
            if let result = result {
